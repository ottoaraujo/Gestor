...
class WPTelegram_Post_Handler {
	...
	public function handle_save_post( $post_id, $post ) {
	    ...
	    if ( ! $is_cron_or_cli ) {
	    	...
	    	if ( ! $user_has_permission ) {
	    		if ( 'page' == $post->post_type && ! ( current_user_can( 'edit_page', $post_id ) ) ) { // Return if user has not permissions to edit pages
					return;
				} elseif ( ! ( current_user_can( 'edit_post', $post_id ) ) ) { // Return if user has not permissions to edit posts
					return;
				}
	    	}
	    }
	    ...
	    $should_be_sent = $this->should_post_be_sent( $post, $send_message );
	    $should_be_sent = apply_filters( 'wptelegram_should_post_be_sent', $should_be_sent, $post );
	...
	private function prepare_message( $post, $post_id, $template ) {
		...
		$short_url = apply_filters( 'wptelegram_post_short_url', wp_get_shortlink( $post_id ), $post );
		
		$full_url = apply_filters( 'wptelegram_post_full_url', urldecode_deep( get_permalink( $post_id ) ), $post );
		...
		if ( preg_match_all( '/(?<=\{\[)[a-z0-9_]+?(?=\]\})/iu', $text, $matches ) ) {
				...
				if ( taxonomy_exists( $taxonomy ) ) {
					...
					if ( ! empty( $terms ) ) {
						...
						if ( is_taxonomy_hierarchical( $taxonomy ) ) {
							...
							$replace = '#'.implode( ' #', $names );
							// replace spaces by underscore
							$replace = preg_replace( '/\s+(?!#)/u', '_', $replace );
						}
					}
				}
	...
	private function add_reply_markup( &$method_params ) {
		...
		$inline_url_button = $this->options['message']['inline_url_button'];
		$inline_button_text = $this->options['message']['inline_button_text'];
		...
		if ( 'on' == $inline_url_button ) {
			...
			$inline_keyboard = apply_filters( 'wptelegram_post_inline_keyboard', $inline_keyboard, $this->post, $method_params );
    ...
    private function handle_wp_error( $WP_Error ) {
    	set_transient( 'wptelegram_http_error', $WP_Error, 45 );
